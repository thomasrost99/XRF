...
class GraphPage(QtWidgets.QWizardPage):
    def __init__(self, parent=None):
        ...
        label = QtWidgets.QLabel("Select a graph to view")
    ...
    def initializePage(self):
        ...
        if not path.exists(dir + '/GraphImages/'):
            makedirs(dir + '/GraphImages/')

        # empties the directory
        filelist = [f for f in listdir(
            dir + "/GraphImages/") if f.endswith(".png")]
        for f in filelist:
            remove(path.join(dir + "/GraphImages/", f))
        ...
        try:
            ...
            for el in energy_levels:
                ...
                if(energy_levels.index(el) == 0):
                    ...
                    xrf_data = pd.merge(xrf_data, el_temp_df, how='outer', on=[
                                        'Site', 'Hole', 'Core', 'Type', 'Section', "Interval"])
            ...
            for element in self.elements:
                ...
                conc_data[element+'/' +
                          base_elem] = np.log(conc_data[element]/conc_data[base_elem])
                xrf_data['ln('+element+'/'+base_elem +
                         ')'] = np.log(xrf_data[element]/xrf_data[base_elem])

                # Finding Equivalent data
                elem_base_master = pd.DataFrame(columns=['Conc', 'Scanner'])
                for i in conc_data.index:
                    hole_scanner = xrf_data[xrf_data['Hole']
                                            == conc_data['Hole'][i]]
                    core_scanner = hole_scanner[hole_scanner['Core']
                                                == conc_data['Core'][i]]
                    section_scanner = core_scanner[core_scanner['Section']
                                                   == conc_data['Section'][i]]
                    interval_scanner = section_scanner[section_scanner['Interval']
                                                       == conc_data['Interval'][i]]
                    if interval_scanner.shape[0] == 0:
                        interval_scanner = section_scanner[section_scanner['Interval']
                                                           == conc_data['Interval'][i]+1]
                        if interval_scanner.shape[0] == 0:
                            interval_scanner = section_scanner[section_scanner['Interval']
                                                               == conc_data['Interval'][i]+2]
                            if interval_scanner.shape[0] == 0:
                                continue
                    elem_base_master = elem_base_master.append({'Conc': conc_data[element+'/'+base_elem][i], 'Scanner': np.mean(interval_scanner['ln('+element+'/'+base_elem+')'])},
                                                               ignore_index=True)
                elem_base_master.columns = [
                    'ln('+element+'/'+base_elem+')_Conc', 'ln('+element+'/'+base_elem+')_XRF']
                ...
                if elem_base_master_final.shape[0] == 0:
                    continue
                X = np.array(
                    elem_base_master_final['ln('+element+'/'+base_elem+')_XRF']).reshape(-1, 1)
                Y = np.array(
                    elem_base_master_final['ln('+element+'/'+base_elem+')_Conc']).reshape(-1, 1)

                if(majorAxisRegressionSelected):
                    results = regress2(
                        X, Y, _method_type_2="reduced major axis")
                    slope = 0
                    intercept = 0
                    r_score = 0
                    ...
                    log_predicted = np.array(
                        xrf_data['ln('+element+'/'+base_elem+')']).reshape(-1, 1)*slope + intercept
                    ...
                    log_predicted = np.array(
                        xrf_data['ln('+element+'/'+base_elem+')']).reshape(-1, 1)*regr.coef_[0][0] + regr.intercept_[0]

                predicted_ratio = np.exp(log_predicted)
                output_data['predicted_ln('+element +
                            '/'+base_elem+')'] = log_predicted
                output_data['predicted_'+element +
                            '/'+base_elem] = predicted_ratio
        ...
        for plot in dict_for_plots:
            fig = plt.figure()
            plt.scatter(dict_for_plots[plot]["x_val"],
                        dict_for_plots[plot]["y_val"], color='b')
            plt.plot(dict_for_plots[plot]["x_val"], dict_for_plots[plot]["coef"] *
                     dict_for_plots[plot]["x_val"] + dict_for_plots[plot]["intercept"], color='k')
            plt.xlabel("Log Ratio of XRF Scanner")
            plt.ylabel("Log Ratio of Concentration")
            plt.title(str(plot))
            r = round(dict_for_plots[plot]["r_score"], 5)
            plt.text(0.1, 0.9, "r^2 = {}".format(
                r), transform=plt.gca().transAxes)
            eq = "y = " + str(round(dict_for_plots[plot]["coef"], 2)) + "x + " + str(
                round(dict_for_plots[plot]["intercept"], 2))
        ...
        onlyfiles = [f for f in listdir(
            dir + "/GraphImages/") if path.isfile(path.join(dir + "/GraphImages/", f))]
    ...
    def onChanged(self, text):
        ...
        self.pic = QtGui.QPixmap(dir + "/GraphImages/" + str(text))
        self.picLabel = QtWidgets.QLabel()
        self.picLabel.setPixmap(self.pic)
        self.layout.addWidget(self.picLabel)
